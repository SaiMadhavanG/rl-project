...
class Weight_assigner():
    ...
    def __init__(self, _replay_buffer: ReplayBuffer, iteration_num, _tde_factor, _reward_factor, _estimated_return_factor, _fr_ratio_current_factor, _fr_ratio_global_factor, _trace_factor, _staleness_factor):
    ...
    def fr_ratio_current_func(self, fr_ratio_current):
        return self.fr_ratio_current_factor * fr_ratio_current                  # TODO : should be inversely. So update this function
    
    def fr_ratio_global_func(self, fr_ratio_global):
        return self.fr_ratio_global_factor * fr_ratio_global                  # TODO : should be inversely. So update this function
    ...
    def without_trace_weight(self, chunk: Chunk):
        return self.tde_func(chunk.tde) + self.reward_func(chunk.rewards) + self.estimated_return_func(chunk.estimated_return) +  self.fr_ratio_current_func(chunk.frequency_ration_current)  +  self.fr_ratio_global_func(chunk.frequency_ration_global) + self.lastSampled_func(chunk.lastSampled)
    ...
    def set_weights(self, trace_multiplier = 0.1):
        ...
        for i in range(-1, -replay_size, -1):
            ...
            previous_chunk_without_trace_weight = self.without_trace_weight(previous_chunk)

            if (previous_chunk.episode_id == current_chunk.episode_id):
                previous_chunk.set_weight(previous_chunk_without_trace_weight +  self.trace_func(trace_multiplier * current_chunk.weight))
            else:
